        ...
        connection.query('SELECT * FROM student WHERE email = ?',req.body.email, async (error,results)=>{
            ...
            if(ArrayData[0]==null){
                if(checkEmail[0]==null){
                    connection.query('INSERT INTO student (id,name,address,email,phone,password) VALUES ?',[value1]);
...
exports.searchOneStudent = async function searchOneStudent (req,res) {
    ...
    ' WHERE id = ? OR name = ? OR status = ? OR group_name = ? OR year = ? OR FromYear = ? OR ToYear = ?',
...
exports.searchByCatagory = async function searchByCatagory (req,res) {
    if(req.body.group_name=='All'){
        connection.query('SELECT * FROM student s JOIN student_yg yg ON s.id = yg.student_id JOIN yg g ON yg.yg_id = g.yg_id JOIN year y ON g.year_id = y.year_id'+
        ' WHERE status = ? AND year = ? AND FromYear = ? AND ToYear = ?',[req.body.status,req.body.year,req.body.FromYear,req.body.ToYear],async (error,results)=>{
            if (error) throw error;
            if(results.length==0){
                res.send("Do not have any data related to what you're searching !!!")
            }else{
        ...
        connection.query('SELECT * FROM student s JOIN student_yg yg ON s.id = yg.student_id JOIN yg g ON yg.yg_id = g.yg_id JOIN year y ON g.year_id = y.year_id'+
        ' WHERE status = ? AND group_name = ? AND year = ? AND FromYear = ? AND ToYear = ?',[req.body.status,req.body.group_name, req.body.year,req.body.FromYear,req.body.ToYear],
        async (error,results)=>{
            if (error) throw error;
            if(results.length==0){
                res.send("Do not have any data related to what you're searching !!!")
